...
def covid_city_stats(update, context):
    ...
    try:
        ...
        user_input = city_stats_cmd[1]
        city_stats = gccs.get_covid_city_stats(user_input)
        ...
        log_action(
            ...
            update.message.from_user.last_name, user_input
            )
...
def openai_jokes(update, context):
    ...
    try:
        aux = update.message.text
        user_input = aux.split(' ',1)[1]
        completion = call_openai.req_openai(user_input)
...
def log_action(chat_id, title, chat_type, user_id , username, first_name, last_name, user_input):
    n0w = datetime.now()
    log = {"chat_id": None, "chat_name": None, "chat_type": None, "user_id": None, "username": None, "first_name": None, 'last_name': None, "user_input": None, "datetime": None}
    log['chat_id'] = chat_id
    log['chat_name'] = title
    ...
    log['user_input'] = user_input.upper()
    log['datetime'] = str(n0w)
    logs = json.dumps(log)
    f = open("user_actions.log","a")
    f.write(logs + "\n")
    f.close()
